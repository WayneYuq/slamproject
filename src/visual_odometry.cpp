...
bool VisualOdometry::addFrame ( Frame::Ptr frame )
{
  switch ( state_ )
  {
    ...
    case OK:
    {
      ...
      {
	curr_->T_c_w_ = T_c_w_estimated_ * ref_-> T_c_w_; // T_c_w_ = T_c_r * T_r_w
	optimizeMap();
	num_lost_ = 0;
...
void VisualOdometry::featureMatching()
{
  ...
  for ( auto& allponits : map_->map_points_ )
  {
    MapPoint::Ptr& p = allponits.second;
...
bool VisualOdometry::checkEstimatedPose()
{
  ...
  SE3 T_c_r = ref_->T_c_w_ * T_c_w_estimated_.inverse();
  Sophus::Vector6d d = T_c_r.log();
  if (d.norm() > 5.0)
  {
